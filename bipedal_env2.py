...
class BipedalWalker(gym.Env, EzPickle):
    ...
    def _generate_terrain(self, hardcore):
        GRASS, STUMP, STAIRS, PIT, SLOPE, _STATES_ = range(6)
        state = GRASS
        velocity = 0.0
        y = TERRAIN_HEIGHT
        ...
        for i in range(TERRAIN_LENGTH):
            ...
            if state == GRASS and not oneshot:
                ...
                if i > TERRAIN_STARTPAD:
                    velocity += self.np_random.uniform(-1, 1) / SCALE  # Less steep
                y += velocity

            # Pit obstacles
            elif state == PIT and oneshot:
                counter = self.np_random.integers(2, 4)  # Smaller pits
                poly = [
                    (x, y),
                    (x + TERRAIN_STEP, y),
                    (x + TERRAIN_STEP, y - 3 * TERRAIN_STEP),
                    (x, y - 3 * TERRAIN_STEP),
                ]
            ...
            # Stump obstacles
                ...
                counter = self.np_random.integers(1, 2)  # Smaller stumps
                poly = [
                    (x, y),
            ...
            # Stairs obstacles
                ...
                stair_width = self.np_random.integers(3, 5)
            ...
            # Slopes
                ...
                slope_angle = self.np_random.uniform(-0.3, 0.3)  # Adjust slope steepness
                y += slope_angle * TERRAIN_STEP

            # Append the current y-coordinate
